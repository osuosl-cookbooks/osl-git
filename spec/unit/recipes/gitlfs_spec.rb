...
describe 'osl-git::gitlfs' do
  ALL_PLATFORMS.each do |p|
    context "#{p[:platform]} #{p[:version]}" do
      ...
      case p
      when ALMA_8
        it { is_expected.to add_osl_packagecloud_repo('github/git-lfs') }
      when ALMA_9
      ...
      case p
      when *ALL_DEBIAN
        it { is_expected.to periodic_apt_update('osl-git-lfs') }
      else
        it { is_expected.to_not periodic_apt_update('osl-git-lfs') }
      end
      ...
      context 'setting arch to aarch64' do
        ...
        it { is_expected.to_not add_osl_packagecloud_repo('github/git-lfs') }
        case p
        when ALMA_8
          it do
            is_expected.to create_yum_repository('git-lfs')
              .with(
        ...
        when ALMA_9
          it { is_expected.to remove_yum_repository 'git-lfs' }
        end
      end
